...
import '../stylesheets/featuredCarousel.css';
      ...
      {isLoading ? (
        ...
        <>
          <div className='carousel-1'>
            {[...ads, ...ads].filter((ad, index) => index % 2 === 0).map((ad, index) => (
              <div
                key={index}
                className={`carousel-slide-1 ${index === currentIndex1 ? 'active' : ''}`}
              >
                <img src={`http://localhost:4000/${ad.image}`} alt={`Image ${index + 1}`} className='carousel-image-1' />
              </div>
            ))}
          </div>
          <div className='carousel-1'>
            {[...ads, ...ads].filter((ad, index) => index % 2 === 1).map((ad, index) => (
              <div
                key={index}
                className={`carousel-slide-1 ${index === currentIndex1 ? 'active' : ''}`}
              >
